...
async function performLLMExtraction(systemPrompt, userQuery, model) {
    if (!openaiClient) {
        // This should never happen
        console.error('No OpenAI client initialized. Re-set OpenAI credentials and try again.');
        return {};
    }
    ...
    while (attempt < 3 && !extractionTerminated) {
        try {
            ...
            // handle rate limit if present in header
            const respHeaders = response.headers || {};
            handleRateLimitHeaders(respHeaders);
            ...
            if (match) {
                try {
                    ...
                    console.warn(`Failed to parse JSON on attempt #${attempt}`, err);
                }
            }
        } catch (err) {
            if (err.response && (err.response.status === 401 || err.response.status === 403)) {
                ...
                // Remove credentials, prompt user
                await handleAuthError();
                throw authError;
            }
            console.error(`Error on extraction attempt #${attempt}:`, err);
        }
        attempt++;
    }
...
async function handleSubmitExtraction() {
    ...
    try {
        ...
        for (const report of reports) {
            ...
            // System prompt + report content
            const developerPrompt = `<instructions>\n${systemPrompt}\n</instructions>\n\n<report>\n${report.content}\n</report>`;
